...
if ($SHOW === TRUE) {
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . "comments` WHERE content_id = '" . intval($_GET['id']) . "' AND content_type='dl' ORDER BY comment_date ASC");
    ...
    while ($comment_arr = $index->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($comment_arr['comment_poster_id'] != 0) {
            $index2 = $FD->db()->conn()->query('SELECT `user_name`, `user_is_admin`, `user_is_staff`, `user_group` FROM `' . $FD->db()->getPrefix() . 'user` WHERE user_id = ' . $comment_arr['comment_poster_id']);
    ...
    $form_spam->tag('captcha_url', url('captcha', array('i' => generate_pwd(8)), true));
    ...
    $presence = $FD->db()->conn()->query('SELECT COUNT(dl_id) FROM `' . $FD->db()->getPrefix() . 'dl` WHERE dl_id = ' . $_GET['id'] . ' AND dl_open = 1');
