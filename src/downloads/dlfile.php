...
$index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "dl WHERE dl_id = $_GET[id] AND dl_open = 1");
...
if ($dl_arr !== false) {
    ...
    // Username auslesen
    $index = $FD->db()->conn()->query('SELECT user_name FROM ' . $FD->db()->getPrefix() . "user WHERE user_id = $dl_arr[user_id]");
    ...
    $index3 = $FD->db()->conn()->query('SELECT cat_name FROM ' . $FD->db()->getPrefix() . "dl_cat WHERE cat_id = '$dl_arr[cat_id]'");
    ...
    // Files auslesen
    $index = $FD->db()->conn()->query('SELECT COUNT(*) FROM ' . $FD->db()->getPrefix() . "dl_files WHERE dl_id = $dl_arr[dl_id] $dl_use");
    $num_rows = $index->fetchColumn();
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "dl_files WHERE dl_id = $dl_arr[dl_id] $dl_use");
    if ($index !== false) {
    ...
    $cc = $FD->db()->conn()->query('SELECT COUNT(comment_id) AS cc FROM `' . $FD->db()->getPrefix() . "comments` WHERE content_type='dl' AND content_id='" . $dl_arr['dl_id'] . "'");
