...
function action_comments_select($DATA)
{
    ...
    $index = $FD->db()->conn()->query("SELECT COUNT(comment_id) AS 'number' FROM " . $FD->env('DB_PREFIX') . 'comments WHERE content_id = ' . $DATA['news_id'] . ' AND content_type=\'news\'');
    ...
    if ($number >= 1) {
        ...
        while ($comment_arr = $index->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($comment_arr['comment_poster_id'] != 0) {
                $index2 = $FD->db()->conn()->query('SELECT user_name FROM ' . $FD->env('DB_PREFIX') . 'USER WHERE user_id = ' . $comment_arr['comment_poster_id'] . "");
...
if (
    ...
    try {
        ...
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->env('DB_PREFIX') . "news_links WHERE `news_id` = '" . $newsid . "'");

        // Insert Links into database
        if (!is_array($_POST['link_name']))
            $_POST['link_name'] = array();
        $stmt = $FD->db()->conn()->prepare('INSERT INTO ' . $FD->env('DB_PREFIX') . 'news_links SET news_id = ' . $newsid . ', link_name = ?, link_url = ?, link_target = ?');
...
elseif (
    ...
    if ($_POST['news_delete'] == 1) {
        try {
            ...
            // delete all links
            $FD->db()->conn()->exec('DELETE FROM ' . $FD->env('DB_PREFIX') . 'news_links WHERE `news_id` IN (' . implode(',', $_POST['news_id']) . ')');
            // delete all comments
            $comment_rows = $FD->db()->conn()->exec('DELETE FROM ' . $FD->env('DB_PREFIX') . 'comments WHERE `content_id` IN (' . implode(',', $_POST['news_id']) . ") AND content_type='news'");

            // update counter
            try {
                $FD->db()->conn()->exec('UPDATE `' . $FD->env('DB_PREFIX') . 'counter` SET `news` = `news` - ' . $num . ', `comments` - ' . $comment_rows . '  WHERE `id` = 1');
            } catch (Exception $e) {
            }
...
if (isset($_POST['news_id']) && isset($_POST['news_action'])) {
    ...
    if ($_POST['news_action'] == 'edit' && count($_POST['news_id']) == 1) {
        ...
        if (isset($_POST['sended'])) {
            ...
            // Get User name
            $_POST['user_name'] = $FD->db()->conn()->query('SELECT user_name FROM ' . $FD->env('DB_PREFIX') . 'USER WHERE user_id=' . intval($_POST['user_id']) . ' LIMIT 1');
            ...
            $links = $FD->db()->conn()->query('SELECT link_name, link_url, link_target FROM ' . $FD->env('DB_PREFIX') . 'news_links
...
if ($FILE_SHOW_START) {
    ...
    try {
        ...
        if ($searched) {
            ...
            $total_entries = $FD->db()->conn()->query('SELECT COUNT(*) FROM ' . $FD->env('DB_PREFIX') . 'news ' . $where);
        ...
        while (true) {
            ...
            switch ($config_arr['acp_view']) {
                ...
                case 1:
                    $news = $FD->db()->conn()->query('SELECT news_id, cat_id, user_id, news_title, news_date, news_text
                                FROM ' . $FD->env('DB_PREFIX') . 'news WHERE news_id = ' . intval($found['id']) . ' LIMIT 1');
            ...
            if (in_array($config_arr['acp_view'], array(1, 2))) {
                //get additional data
                $user = $FD->db()->conn()->query('SELECT user_name FROM ' . $FD->env('DB_PREFIX') . 'USER WHERE user_id=' . intval($news['user_id']) . ' LIMIT 1');
                $user = $user->fetchColumn();
                $cat = $FD->db()->conn()->query('SELECT cat_name FROM ' . $FD->env('DB_PREFIX') . 'news_cat WHERE cat_id=' . intval($news['cat_id']) . ' LIMIT 1');
                $cat = $cat->fetchColumn();
                $num_comments = $FD->db()->conn()->query('SELECT COUNT(comment_id) FROM ' . $FD->env('DB_PREFIX') . 'comments
                                    WHERE `content_id` = ' . intval($news['news_id']) . " AND `content_type` = 'news'");
                $num_comments = $num_comments->fetchColumn();
                $num_links = $FD->db()->conn()->query('SELECT COUNT(link_id) FROM ' . $FD->env('DB_PREFIX') . 'news_links
            ...
            switch ($config_arr['acp_view']) {
                case 1:
                    $entries[] = $adminpage->get('list_entry_full');
                    break;
                case 2:
                    $entries[] = $adminpage->get('list_entry_extended');
                    break;
                default:
                    $entries[] = $adminpage->get('list_entry_simple');
                    break;
            }
        }
