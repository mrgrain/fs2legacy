...
if ($FD->cfg('goto') == 'articles') {
    ...
    // Load Article Data from DB
    $article_arr = $FD->db()->conn()->query(
        'SELECT * FROM ' . $FD->env('DB_PREFIX') . "articles
...
if (empty($article_arr)) {
    ...
    $user_arr = $FD->db()->conn()->query('SELECT user_id, user_name FROM ' . $FD->env('DB_PREFIX') . 'USER WHERE user_id =' . intval($article_arr['article_user']) . ' LIMIT 1');
