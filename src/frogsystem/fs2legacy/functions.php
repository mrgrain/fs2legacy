...
function get_pagenav_start($NUM_OF_ENTRIES, $ENTRIES_PER_PAGE, $START)
{
    ...
    if ($START > $NUM_OF_ENTRIES) {
        $START = $NUM_OF_ENTRIES - $ENTRIES_PER_PAGE;
    }
    ...
    if ($NEWSTART > $NUM_OF_ENTRIES) {
        $NEWSTART = $NUM_OF_ENTRIES - $ENTRIES_PER_PAGE;
    }
    ...
    if ($START > 1) {
        $PAGENAV_DATA['old_start_exists'] = TRUE;
    } else {
        $PAGENAV_DATA['old_start_exists'] = FALSE;
    }
    if (($START + $ENTRIES_PER_PAGE) < $NUM_OF_ENTRIES) {
        $PAGENAV_DATA['newpage_exists'] = TRUE;
    } else {
        $PAGENAV_DATA['newpage_exists'] = FALSE;
    }
