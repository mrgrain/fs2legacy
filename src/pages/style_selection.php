...
// other styles not allowed
if (!$FD->cfg('allow_other_designs')) {
    $template = sys_message($FD->text('frontend', 'sysmessage'), $FD->text('frontend', 'style_selection_not_allowed'));
    ...
    $current = isset($_COOKIE['style']) ? htmlentities($_COOKIE['style']) : $FD->text('frontend', 'none') . ' (' . htmlentities($FD->cfg('style')) . ')';
