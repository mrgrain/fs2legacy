...
////////////////////////////
    ...
    switch ($_GET['sort']) {
        ...
        case 'type': {
            $index = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . 'poll` ORDER BY `poll_type` ' . $_GET['order'] . ', `poll_quest` ASC');
            break;
        }
        case 'start_date': {
            $index = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . 'poll` ORDER BY `poll_start` ' . $_GET['order'] . ', `poll_quest` ASC');
            break;
        }
        case 'end_date': {
            $index = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . 'poll` ORDER BY `poll_end` ' . $_GET['order'] . ', `poll_quest` ASC');
            break;
        }
    }
