...
// Get Number of Users
$index = $FD->db()->conn()->query('SELECT COUNT(`user_id`) AS num_users FROM `' . $FD->db()->getPrefix() . 'user`');
...
switch ($_GET['sort']) {
    ...
    case 'mail':
        switch ($_GET['order']) {
            case 0:
                $query .= '`user_show_mail` DESC, `user_mail` ASC';
                break 2;
            default:
                $query .= '`user_show_mail` DESC, `user_mail` DESC';
                break 2;
        }
    case 'reg_date':
        switch ($_GET['order']) {
            case 0:
                $query .= '`user_reg_date` DESC';
                break 2;
            default:
                $query .= '`user_reg_date` ASC';
                break 2;
        }
    case 'num_news':
        switch ($_GET['order']) {
            case 0:
                $query .= 'user_num_news DESC';
                break 2;
            default:
                $query .= 'user_num_news ASC';
                break 2;
        }
    case 'num_comments':
        switch ($_GET['order']) {
            case 0:
                $query .= 'user_num_comments DESC';
                break 2;
            default:
                $query .= 'user_num_comments ASC';
                break 2;
        }
    case 'num_articles':
        switch ($_GET['order']) {
            case 0:
                $query .= 'user_num_articles DESC';
                break 2;
            default:
                $query .= 'user_num_articles ASC';
                break 2;
        }
    case 'num_downloads':
        switch ($_GET['order']) {
            case 0:
                $query .= 'user_num_downloads DESC';
                break 2;
            default:
                $query .= 'user_num_downloads ASC';
                break 2;
        }
}
