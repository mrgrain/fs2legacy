...
if (isset($_POST['sended']) && isset($_POST['cat_action']) && $_POST['cat_action'] == "import") {
    ...
    if (count($_POST['image']) > 0 && !is_writable(CIMG_PATH)) {
        ...
        $count = 0;
        $stmt = $FD->db()->conn()->prepare('INSERT INTO `' . $FD->db()->getPrefix() . 'cimg` (`name`, `type`, `hasthumb`, `cat`) VALUES (?, ?, ?, ?)');
...
if (!isset($_POST['cat_action']) || !isset($_POST['sended'])) {
    $qry = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . 'cimg`');
    $img = array();
    ...
    $ordner = opendir(UPLOAD_PATH); // gib hier den gew�nschten Pfad an
