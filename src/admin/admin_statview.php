...
$index = $FD->db()->conn()->query('SELECT s_year FROM ' . $FD->db()->getPrefix() . 'counter_stat ORDER BY s_year LIMIT 1');
$dbfirstyear = $index->fetchColumn();

// find first month
$index = $FD->db()->conn()->query('SELECT s_month FROM ' . $FD->db()->getPrefix() . "counter_stat WHERE s_year = $dbfirstyear ORDER BY s_month LIMIT 1");
$dbfirstmonth = $index->fetchColumn();

echo '<a href="?go=stat_view&s_year=' . $dbfirstyear . '&s_month=' . $dbfirstmonth . '">';
if ($_GET['s_year'] == $dbfirstyear) {
    echo '<b>';
}
echo $dbfirstyear;
if ($_GET['s_year'] == $dbfirstyear) {
...
$index = $FD->db()->conn()->query(
    'SELECT s_hits
              FROM ' . $FD->db()->getPrefix() . "counter_stat
        ...
        echo '
                                    <tr>
                                        ...
                                        <td ' . $class . 'align="left" style="font-size:1pt;">
                                            <img border="0" src="assets/images/cvisits.gif" height="4" width="' . round($visitswidth) . '"><br>
                                            <img border="0" src="assets/images/chits.gif" height="4" width="' . round($hitswidth) . '">
                                        </td>
                                    </tr>
        ';
    } else {
        ...
        echo '
                                    <tr>
                                        ...
                                        <td class="n" align="left" style="font-size:1pt;" class="bottom">
                                            ...
                                            style="cursor:pointer; padding-left:2px; padding-right:2px;" border="0" src="assets/images/cdiag.gif">
                                            <img border="0" src="assets/images/null.gif" height="4" width="1"><br>
                                            <img border="0" src="assets/images/cvisits.gif" height="4" width="' . round($visitswidth) . '"><br>
                                            <img border="0" src="assets/images/chits.gif" height="4" width="' . round($hitswidth) . '">
                                        </td>
                                    </tr>
...
// Day of most hits
$index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . 'counter_stat ORDER BY s_hits DESC LIMIT 1');
$mosthits = $index->fetch(PDO::FETCH_ASSOC);

// Day of most visits
$index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . 'counter_stat ORDER BY s_visits DESC LIMIT 1');
