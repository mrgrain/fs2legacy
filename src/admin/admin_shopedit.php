...
if (isset($_POST['title']) && isset($_POST['url']) && isset($_POST['preis']) && $_POST['sended'] == 'edit'
    ...
    if (isset($_POST['delartikel'])) {
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . "shop WHERE artikel_id = $_POST[artikelid]");
...
if (isset($_POST['artikelid'])) {
    ...
    settype($_POST['artikelid'], 'integer');
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "shop WHERE artikel_id = $_POST[artikelid]");
