...
if ((isset($_POST['title']) AND $_POST['title'] != '')
    ...
    $stmt = $FD->db()->conn()->prepare(
        'INSERT INTO
                 ' . $FD->db()->getPrefix() . "press (press_title,
                           press_url,
                           press_date,
                           press_intro,
                           press_text,
...
if (true) {
    ...
    if (isset($_POST['sended']) && $_POST['sended'] == 'add') {
        ...
        settype($date['month'], 'integer');
        if ($date['month'] == 0) {
            $date['month'] = '';
        }
        $date['jahr'] = $_POST['year'];
        settype($date['year'], 'integer');
        if ($date['year'] == 0) {
            $date['year'] = '';
        }
    ...
    while ($cat_arr = $index->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($cat_arr['id'] == $press_arr['press_cat']) {
            echo ' selected="selected"';
        }
    ...
    while ($lang_arr = $index->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($lang_arr['id'] == $press_arr['press_lang']) {
            echo ' selected="selected"';
        }
