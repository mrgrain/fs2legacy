...
function action_delete_get_data($IDS)
{
    ...
    foreach ($IDS as $NEWS_ID) {
        ...
        $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "news WHERE news_id = '" . $NEWS_ID . "'");
...
function action_comments_select($DATA)
{
    ...
    $index = $FD->db()->conn()->query("SELECT COUNT(comment_id) AS 'number' FROM " . $FD->db()->getPrefix() . 'comments WHERE content_id = ' . $DATA['news_id'] . ' AND content_type=\'news\'');
...
if (
    ...
    try {
        ...
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . "news_links WHERE `news_id` = '" . $newsid . "'");

        // Insert Links into database
        if (!is_array($_POST['link_name']))
            $_POST['link_name'] = array();
        $stmt = $FD->db()->conn()->prepare('INSERT INTO ' . $FD->db()->getPrefix() . 'news_links SET news_id = ' . $newsid . ', link_name = ?, link_url = ?, link_target = ?');
...
elseif (
    ...
    if ($_POST['news_delete'] == 1) {
        try {
            ...
            //delete news
            $num = $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . 'news WHERE `news_id` IN (' . implode(',', $_POST['news_id']) . ')');

            // Delete from Search Index
            delete_search_index_for_one($_POST['news_id'], 'news');

            // delete all links
            $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . 'news_links WHERE `news_id` IN (' . implode(',', $_POST['news_id']) . ')');
            // delete all comments
            $comment_rows = $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . 'comments WHERE `content_id` IN (' . implode(',', $_POST['news_id']) . ") AND content_type='news'");

            // update counter
            try {
                $FD->db()->conn()->exec('UPDATE `' . $FD->db()->getPrefix() . 'counter` SET `news` = `news` - ' . $num . ', `comments` - ' . $comment_rows . '  WHERE `id` = 1');
            } catch (Exception $e) {
            }
...
if (isset($_POST['news_id']) && isset($_POST['news_action'])) {
    ...
    if ($_POST['news_action'] == 'edit' && count($_POST['news_id']) == 1) {
        ...
        if (isset($_POST['sended'])) {
            ...
            $links = $FD->db()->conn()->query('SELECT link_name, link_url, link_target FROM ' . $FD->db()->getPrefix() . 'news_links
...
if ($FILE_SHOW_START) {
    ...
    try {
        ...
        if ($searched) {
            ...
            $total_entries = $FD->db()->conn()->query('SELECT COUNT(*) FROM ' . $FD->db()->getPrefix() . 'news ' . $where);
        ...
        while (true) {
            ...
            switch ($config_arr['acp_view']) {
                ...
                case 1:
                    $news = $FD->db()->conn()->query('SELECT news_id, cat_id, user_id, news_title, news_date, news_text
                                FROM ' . $FD->db()->getPrefix() . 'news WHERE news_id = ' . intval($found['id']) . ' LIMIT 1');
            ...
            if (in_array($config_arr['acp_view'], array(1, 2))) {
                ...
                $cat = $cat->fetchColumn();
                $num_comments = $FD->db()->conn()->query('SELECT COUNT(comment_id) FROM ' . $FD->db()->getPrefix() . 'comments
                                    WHERE `content_id` = ' . intval($news['news_id']) . " AND `content_type` = 'news'");
                $num_comments = $num_comments->fetchColumn();
                $num_links = $FD->db()->conn()->query('SELECT COUNT(link_id) FROM ' . $FD->db()->getPrefix() . 'news_links
