...
if (isset($_POST['dledit']) && isset($_POST['title']) && isset($_POST['text']) && !isset($_POST['files_add'])) {
    ...
    if (isset($_POST['deldl'])) {
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . "dl WHERE dl_id = '$_POST[editdlid]'");
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . "dl_files WHERE dl_id = '$_POST[editdlid]'");
        ...
        $stmt = $FD->db()->conn()->prepare('UPDATE ' . $FD->db()->getPrefix() . "dl
        ...
        for ($i = 0; $i < count($_POST['fname']); $i++) {
            if (isset($_POST['delf'][$i]) && $_POST['delf'][$i] != 0) {
                ...
                $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . 'dl_files WHERE file_id = ' . $_POST['delf'][$i]);
            } else {
                ...
                if ($_POST['fnew'][$i] == 1 && $_POST['fname'][$i] != '') {
                    $stmt = $FD->db()->conn()->prepare(
                        'INSERT INTO ' . $FD->db()->getPrefix() . "dl_files
                    ...
                    $stmt = $FD->db()->conn()->prepare('UPDATE ' . $FD->db()->getPrefix() . "dl_files
...
if (isset($_POST['dlid']) || isset($_POST['dlid']) && isset($_POST['files_add']) && isset($_POST['optionsadd'])) {
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "dl WHERE dl_id =  '$_POST[dlid]'");
    ...
    $index = $FD->db()->conn()->query('SELECT COUNT(*) FROM ' . $FD->db()->getPrefix() . "dl_files WHERE dl_id = '$_POST[dlid]'");
    $rows = $index->fetchColumn();
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "dl_files WHERE dl_id = '$_POST[dlid]' ORDER BY file_id");
    for ($i = 0; $i < $rows; $i++) {
        $row = $index->fetch(PDO::FETCH_ASSOC);
    ...
    ';
    $index = $FD->db()->conn()->query('SELECT `ftp_id` FROM ' . $FD->db()->getPrefix() . "ftp WHERE `ftp_type` = 'dl' LIMIT 0,1");
    ...
    echo '<form action="" method="post">
          ...
          <table border="0" cellpadding="4" cellspacing="0" width="600" class="content">
              ...
              ';
    $comments = $FD->db()->conn()->query('SELECT COUNT(*) FROM `' . $FD->db()->getPrefix() . 'comments` WHERE content_type=\'dl\' AND content_id=\'' . $_POST['dlid'] . "'");
    if ($comments->fetchColumn() == 0) {
        ...
        $comments = $FD->db()->conn()->query('SELECT * FROM `' . $FD->db()->getPrefix() . 'comments` WHERE content_type=\'dl\' AND content_id=\'' . $_POST['dlid'] . "'");
        while ($row = $comments->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($row['comment_poster_id'] != 0) {
                $user = $FD->db()->conn()->query('SELECT user_name FROM `' . $FD->db()->getPrefix() . "user` WHERE user_id='" . $row['comment_poster_id'] . "' LIMIT 1");
...
/////////////////////////////
    ...
    $index = $FD->db()->conn()->query('SELECT dl_id, dl_name, cat_id FROM ' . $FD->db()->getPrefix() . "dl $wherecat ORDER BY dl_name");
    while ($dl_arr = $index->fetch(PDO::FETCH_ASSOC)) {
        $catindex = $FD->db()->conn()->query('SELECT cat_name FROM ' . $FD->db()->getPrefix() . "dl_cat WHERE cat_id = '$dl_arr[cat_id]'");
        $dbcatname = $catindex->fetchColumn();

        $comments = $FD->db()->conn()->query('SELECT COUNT(*) FROM `' . $FD->db()->getPrefix() . 'comments` WHERE content_type=\'dl\' AND content_id=\'' . $dl_arr['dl_id'] . "'");
