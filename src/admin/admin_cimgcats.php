...
if (isset($_POST['add'])) {
    ...
    if (!empty($name)) {
        $stmt = $FD->db()->conn()->prepare('INSERT INTO `' . $FD->db()->getPrefix() . 'cimg_cats` (`name`, `description`) VALUES (?, ?)');
    ...
    if (isset($_POST['cat'])) {
        $count = 0;
        $stmt = $FD->db()->conn()->prepare('UPDATE `' . $FD->db()->getPrefix() . 'cimg_cats` SET `name`=?, `description`=? WHERE `id`=?');
    ...
    if (isset($_POST['cat'])) {
        $count = 0;
        $stmt = $FD->db()->conn()->prepare('UPDATE `' . $FD->db()->getPrefix() . "cimg` SET `cat`=? WHERE `cat`=?");
        foreach ($_POST['cat'] as $cat) {
            $FD->db()->conn()->exec('DELETE FROM `' . $FD->db()->getPrefix() . 'cimg_cats` WHERE `id`=' . intval($cat));
...
FS2_STRING;
$qry = $FD->db()->conn()->query('SELECT COUNT(*) FROM `' . $FD->db()->getPrefix() . 'cimg_cats`');
