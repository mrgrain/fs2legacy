...
$index = $FD->db()->conn()->query("
				...
				FROM " . $FD->db()->getPrefix() . 'screen_cat
				LIMIT 0,1');
...
$index = $FD->db()->conn()->query("
				...
				FROM " . $FD->db()->getPrefix() . 'wallpaper
				LIMIT 0,1');
...
if ($num_gallery_img > 0) {
    $index = $FD->db()->conn()->query("
					...
					FROM " . $FD->db()->getPrefix() . 'screen_cat C, ' . $FD->db()->getPrefix() . 'screen S
...
if ($num_gallery_wp > 0) {
    $index = $FD->db()->conn()->query("
					...
					FROM " . $FD->db()->getPrefix() . 'screen_cat C, ' . $FD->db()->getPrefix() . 'wallpaper W
...
if ($num_dl > 0 && $num_dl_files > 0) {
    $index = $FD->db()->conn()->query("
					...
					FROM " . $FD->db()->getPrefix() . 'dl D, ' . $FD->db()->getPrefix() . 'dl_files F
