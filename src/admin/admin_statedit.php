...
elseif (isset($_POST['do']) && $_POST['do'] == 'sync') {
    ...
    $index = $FD->db()->conn()->query("SELECT COUNT(comment_id) AS 'comments' FROM " . $FD->db()->getPrefix() . 'comments');
    $sync_arr['comments'] = $index->fetchColumn();

    $index = $FD->db()->conn()->query("SELECT COUNT(article_id) AS 'articles' FROM " . $FD->db()->getPrefix() . 'articles');
