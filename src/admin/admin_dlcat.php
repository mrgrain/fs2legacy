...
if (isset($_POST['catname'])) {
    ...
    if (isset($_POST['delcat'])) {
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . 'dl_cat WHERE cat_id = ' . $_POST['catid']);
        systext('Die Kategorie wurde gel&ouml;scht');
    } else {
        settype($_POST['subcatof'], 'integer');

        $stmt = $FD->db()->conn()->prepare(
            'UPDATE ' . $FD->db()->getPrefix() . "dl_cat
...
if (isset($_POST['editcatid'])) {
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "dl_cat WHERE cat_id = '$_POST[editcatid]'");
...
///////////////////////
    ...
    ';
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . 'dl_cat ORDER BY cat_name');
