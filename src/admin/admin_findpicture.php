...
// display images
if (isset($_POST['cat'])) {
    $index = $FD->db()->conn()->prepare('SELECT COUNT(*) FROM ' . $FD->db()->getPrefix() . 'screen WHERE cat_id = ?');
    ...
    if ($num_rows <= 0) {
        ...
        $index = $FD->db()->conn()->prepare('SELECT * FROM ' . $FD->db()->getPrefix() . 'screen WHERE cat_id = ? ORDER BY screen_id DESC');
        $index->execute(array($_POST['cat']));
