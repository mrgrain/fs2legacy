...
if (
    ...
    && $_POST['applet_file'] != ''
    && file_exists(FS2APPLETS . $_POST['applet_file'] . '.php')
) {
    ...
    $index = $FD->db()->conn()->prepare('SELECT `applet_id` FROM `' . $FD->db()->getPrefix() . 'applets` WHERE `applet_file` = ?');
    ...
    if ($row === false) {
        // SQL-Queries
        $stmt = $FD->db()->conn()->prepare('INSERT INTO `' . $FD->db()->getPrefix() . "applets` (
                                  `applet_file`,
                                  `applet_active`,
                                  `applet_include`,
                                  `applet_output`
...
if (isset ($_POST['sended'])) {
    if (isset ($_POST['applet_file']) && $_POST['applet_file'] != '' && !file_exists(FS2APPLETS . $_POST['applet_file'] . '.php')) {
