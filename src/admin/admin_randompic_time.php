...
if (!empty($_POST['screen_id'])
    ...
    if ($startdate < $enddate) {
        $update = 'UPDATE ' . $FD->db()->getPrefix() . "screen_random
...
elseif (isset($_POST['random_action']) && $_POST['random_action'] == 'delete'
    ...
    {
        $FD->db()->conn()->exec('DELETE FROM ' . $FD->db()->getPrefix() . "screen_random WHERE random_id = '$_POST[random_id]'");
...
elseif (isset($_POST['random_action']) && $_POST['random_action'] == 'edit'
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . "screen_random WHERE random_id = $_POST[random_id]");
...
elseif (isset($_POST['random_action']) && $_POST['random_action'] == 'delete'
    ...
    $index = $FD->db()->conn()->query('SELECT screen_name FROM ' . $FD->db()->getPrefix() . "screen WHERE screen_id = $random_arr[screen_id]");
...
if (!isset($_POST['random_id'])) {
    ...
    ';
    $index = $FD->db()->conn()->query('SELECT * FROM ' . $FD->db()->getPrefix() . 'screen_random a, ' . $FD->db()->getPrefix() . 'screen b WHERE a.screen_id = b.screen_id ORDER BY a.end DESC');
